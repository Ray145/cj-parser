...
async function initialise() {
    ...
    try {
        _MONGOOSE_CLIENT = await mongoose.connect(mongoConnString, { useNewUrlParser: true, useUnifiedTopology: true });
    } catch (err) {
        logger.error(`db - mongo - Error while connecting to ${mongoConnString}; with message: ${err.message}`);
        logger.error(err);
        throw new Error(err);
    }
